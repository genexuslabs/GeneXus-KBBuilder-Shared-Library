 ...
 * @param
 */
void sendAlerts(def currentBuildDefinition, def notifyTo, String gxVersion) {
    try {
        def changeLogSet = getChangeLogSet(currentBuildDefinition)
        def icon
        String template
        String templateName
        String buildColor
        ...
        String jobFullDisplayName = currentBuildDefinition.fullDisplayName
        def splitJobDisplayName = currentBuildDefinition.fullDisplayName.split(' » ')
        def jobDisplayName = "${jobFullDisplayName.replace(splitJobDisplayName[splitJobDisplayName.length - 1], '')}"
        switch (currentBuildDefinition.currentResult) {
            case 'SUCCESS':
                icon = "✅"
                buildColor = "green"
                buildResult = "Success"
...
/*        for(def notification in notificationDefinition) {
            if(notification.when.contains(currentBuildDefinition.currentResult)) {
                for(def notificationAction in notification.by) {
...
*/
                            ...
                            template = createTemplate(templateName, [
                                ...
                                "jenkinsDuration"   :   currentBuildDefinition.durationString.replaceAll(' and counting', ''),
                                "changeLogSet"      :   changeLogSet,
                                "cause"             :   currentBuildDefinition.buildCauses[0].shortDescription.replaceAll('\\[',' '),
                                "gxversion"         :   gxVersion
                            ]);

                            emailext body: template,
                                mimeType: 'text/html',
                                subject: "${icon} ${jobDisplayName.toString()} Build #${env.BUILD_NUMBER} » ${currentBuildDefinition.currentResult}",
                                to: "${notifyTo}",
                                replyTo: "${notifyTo}",
                                attachLog: true
                        break
